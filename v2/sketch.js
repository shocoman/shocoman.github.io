...
function setup() {
  ...
  createElement('br');
  frame_rate_slider = createSlider(1, 20, 5); createElement('label', ' <= Speed');
}
...
class Hamster {
  constructor() {
    this.img = loadImage('/chibi.png');

    this.pos = createVector(0, 400);
  ...
  draw() {
    ...
    let size = map(this.vel.y, -13, 5, 0.15, 5);
    image(this.img, 0, 0, width / 4 / size, height / 4 / size);
  }
  ...
  shoot() {
    ...
    launch_power.mult(-12 * width / 600);
    this.vel = launch_power;

    this.rotation_speed = random(0.08, 0.25) * (launch_power.x >= 0 ? 1 : -1);
  }
